...
import { X, Link2, Loader2, ChevronRight, ChevronDown, ExternalLink, GripVertical } from "lucide-react";
...
export function LogDetailsPanel({ log, open, onClose, onSelectRelated }: LogDetailsPanelProps) {
    ...
    useEffect(() => {
        const savedWidth = localStorage.getItem('log-details-panel-width');
        if (savedWidth) {
    ...
    const startResizing = useCallback((mouseDownEvent: React.MouseEvent) => {
        ...
        const onMouseMove = (mouseMoveEvent: MouseEvent) => {
            ...
            currentWidth = Math.min(Math.max(startWidth + delta, 400), window.innerWidth * 0.9);
        ...
        const onMouseUp = () => {
            ...
            localStorage.setItem('log-details-panel-width', String(currentWidth));
        };
    ...
    return (
        <div
            className="border-l border-zinc-800 bg-zinc-950 flex flex-col h-full shrink-0 shadow-2xl relative"
            style={{ width: width, transition: isResizing ? 'none' : 'width 0.3s ease' }}
        >
            {/* Resize Handle */}
            <div
                className="absolute left-0 top-0 bottom-0 w-4 -ml-2 bg-transparent hover:cursor-col-resize z-50 flex items-center justify-center group"
                onMouseDown={startResizing}
            >
                {/* Visual indicator for handle */}
                <div className="h-8 w-4 bg-zinc-900 border border-zinc-800 rounded-none flex items-center justify-center shadow-sm z-50">
            ...
            <div className="p-6 border-b border-zinc-900 flex justify-between items-start">
                <div className="space-y-1 overflow-hidden">
                    ...
                    {traceId && (
                        <div className="flex items-center gap-2 text-xs text-zinc-500 mt-1">
                            ...
                            <span className="font-mono truncate">trace: {traceId}</span>
                            {relatedLoading && (
                                <span className="flex items-center gap-1 text-[10px] text-blue-400/70 ml-2 animate-pulse shrink-0">
                ...
                <Button variant="ghost" size="icon" onClick={onClose} className="text-zinc-500 hover:text-white rounded-none shrink-0 ml-2">
                    <X className="h-4 w-4" />
                </Button>
            </div>
            ...
            <div className="flex-1 overflow-hidden relative">
                <ScrollArea className="h-full">
                    <div className="p-4 space-y-4">
                        ...
                        {/* Event Payload */}
                        <div className="border border-zinc-800 bg-zinc-900/50 p-4 font-mono text-sm overflow-x-auto">
                            <SyntaxHighlighter
                                language="json"
                                style={customStyle}
                                wrapLongLines={false}
