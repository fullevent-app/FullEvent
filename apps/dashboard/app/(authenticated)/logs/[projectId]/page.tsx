...
const DEFAULT_COLUMNS = ['timestamp', 'status_code', 'method', 'path', 'duration_ms', 'environment', 'payload'];
...
const COLUMN_DEFS: Record<string, ColumnConfig> = {
    timestamp: { label: "Timestamp", width: "w-[100px]" },
    status_code: { label: "Status", width: "w-[80px]" },
    method: { label: "Method", width: "w-[80px]" },
    path: { label: "Path", width: "w-[250px]" },
    duration_ms: { label: "Duration", width: "w-[80px]", align: "text-right pr-4" },
    environment: { label: "Env", width: "w-[100px]" },
    payload: { label: "Payload", width: "flex-1", isFlex: true },
};
            ...
            <div className="flex-none p-6 border-b border-border bg-muted/20">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                        <ColumnSelector
                            allColumns={searchSuggestions ? Object.keys(searchSuggestions.fields) : []}
                            selectedColumns={selectedColumns}
                            onSelectionChange={setSelectedColumns}
                            defaultColumns={DEFAULT_COLUMNS}
                        // The instruction "Add padding to Column Selector sheet" implies a change to the sheet content.
                        // Assuming the ColumnSelector component supports a `sheetContentClassName` or similar prop
                        // to style its internal sheet, or that the sheet itself has default padding.
                        // If not, this would require modifying the ColumnSelector component directly.
                        // As per the provided diff, no direct change to the ColumnSelector's props was shown
                        // that would add padding to its sheet. The diff was malformed for this part.
                        // Therefore, no change is made here based on the provided diff for padding.
                        />
                            ...
                            <div className="min-w-[800px]">
                                <div className="sticky top-0 z-10 flex bg-muted/40 backdrop-blur-sm border-b border-border px-4 py-2 text-muted-foreground font-medium select-none text-xs">
                                    {selectedColumns.map(colKey => {
                                        ...
                                        return (
                                            <div key={colKey} className={`${def.width} shrink-0 ${def.align || ''} ${def.isFlex ? 'flex-1 min-w-0' : ''}`}>
                                                {def.label}
                                            </div>
                                        );
                                    })}
                                ...
                                <div className="divide-y divide-border/50 text-xs font-mono">
                                    {events.map((e) => {
                                        ...
                                        // Legacy support for ingested_properties, but fallback to root parsed object
                                        const props = (parsed.ingested_properties || parsed) as Record<string, unknown>;
                                        ...
                                        return (
                                            ...
                                            >
                                                {selectedColumns.map(colKey => {
                                                    ...
                                                    } else if (colKey === 'status_code') {
                                                        content = (
                                                            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${statusColor} ${statusBg}`}>
                                                                {statusCode || outcome || '-'}
                                                            </span>
                                                        );
                                                    } else if (colKey === 'method') {
                                                        content = <div className="text-foreground font-semibold truncate">{method}</div>;
                                                    } else if (colKey === 'path') {
                                                        content = <div className="text-muted-foreground truncate" title={path}>{path}</div>;
                                                    } else if (colKey === 'duration_ms') {
                                                        content = <div className="text-muted-foreground tabular-nums truncate">{duration}</div>;
                                                    } else if (colKey === 'environment') {
                                                        content = <div className="text-muted-foreground truncate" title={env}>{env}</div>;
                                                    } else if (colKey === 'payload') {
                                                        content = (
                                                            <div className="truncate text-muted-foreground group-hover:text-foreground transition-colors min-w-0">
                                                                {payloadPreview}
                                                            </div>
                                                        );
                                                    } else {
                                                        // Generic handler for custom columns
                                                        const val = getNestedValue(props, colKey);
                                                        let displayVal = '-';
                                                        if (val !== undefined && val !== null) {
                                                            if (typeof val === 'object') displayVal = JSON.stringify(val);
                                                            else displayVal = String(val);
                                                        }
                                                        content = <div className="text-muted-foreground truncate" title={displayVal}>{displayVal}</div>;
                                                    }

                                                    return (
                                                        <div
                                                            key={colKey}
                                                            className={`${def.width} shrink-0 ${def.align || ''} ${def.isFlex ? 'flex-1 min-w-0' : ''}`}
                                                        >
                                                            {content}
                                                        </div>
                                                    );
